...
struct HideoutTabView: View {
    ...
    var rollingScorecardWeeks: [Date] {
        let start = DistributionCalendar.mondayStart(of: Date())
        return (0..<4).compactMap { offset in
            Calendar.current.date(byAdding: .weekOfYear, value: -offset, to: start)
        }.reversed()
    }

    func rollingWeekLabel(for weekStart: Date) -> String {
                ...
                if decisionsExpanded {
                        ...
                        CardView {
                            VStack(alignment: .leading, spacing: metrics.blockSpacing) {
                                ...
                                Rectangle().fill(Color.muted.opacity(0.2)).frame(height: 0.5)
                                Text("Growth phase since May 13. Track 30-day avg and band movement — capital decisions from stability data, not crisis framing.")
                                    .font(.system(size: metrics.scaledSize(10), design: .monospaced))
                                    .foregroundColor(.textMuted)
                                    .lineSpacing(2.5)
                            }
        ...
        VStack(spacing: metrics.blockSpacing) {
            ...
            CardView {
                VStack(alignment: .leading, spacing: metrics.blockSpacing) {
                    ...
                    ForEach(rollingScorecardWeeks, id: \.self) { weekStart in
                        rollingWeekRow(weekStart: weekStart)
                        if weekStart != rollingScorecardWeeks.last {
                            Rectangle().fill(Color.muted.opacity(0.15)).frame(height: 0.5)
                        }
                    }
    ...
    func rollingWeekRow(weekStart: Date) -> some View {
        let cal = Calendar.current
        let days = ["M", "T", "W", "Th", "F", "Sa", "Su"]
        return VStack(alignment: .leading, spacing: metrics.rowSpacing) {
            MonoLabel(text: rollingWeekLabel(for: weekStart).uppercased(), color: .textMuted, size: 9)
            HStack(spacing: metrics.rowSpacing) {
                ForEach(days.indices, id: \.self) { i in
                    let columnDate = cal.date(byAdding: .day, value: i, to: weekStart) ?? weekStart
            ...
            let weekEnd = cal.date(byAdding: .day, value: 7, to: weekStart) ?? Date.distantFuture
            let weekShifts = recentShifts.filter { $0.logDate >= weekStart && $0.logDate < weekEnd }
...
// MARK: - FRIDAY SIGNAL CARD (reads DistributionWeek — canonical Signal tab model)

struct FridaySignalCard: View {
    @Query(sort: \DistributionWeek.weekStartDate, order: .reverse) private var weeks: [DistributionWeek]
    @Environment(\.hideoutMetrics) private var metrics

    var latestWeek: DistributionWeek? {
        weeks.first(where: \.fridayLogCompleted) ?? weeks.first
    }
    ...
    var body: some View {
        CardView(style: .secondary) {
            VStack(alignment: .leading, spacing: metrics.blockSpacing) {
                HStack {
                    VStack(alignment: .leading, spacing: metrics.rowSpacing) {
                        MonoLabel(text: "FRIDAY SIGNAL", color: .inkGreen, size: 10)
                        Text("Logged in Signal tab after close · DistributionWeek canonical")
                            .font(.sora(metrics.scaledSize(10), weight: .light)).foregroundColor(.textMuted)
                    }
                    Spacer()
                    MonoLabel(text: "SIGNAL TAB", color: .inkGreen, size: 9)
                }

                if let week = latestWeek, week.fridayLogCompleted {
                    VStack(spacing: 0) {
                        signalTargetRow("HIDEOUT", "Source mentions",         current: week.hideoutSourceMentions, target: 5,  unit: "/wk", color: .warm)
                        Divider().background(Color.muted.opacity(0.12))
                        signalTargetRow("HIDEOUT", "Watermarc redemptions",   current: week.hideoutWatermarcRedemptions, target: 2,  unit: "/wk", color: .inkTeal)
                        Divider().background(Color.muted.opacity(0.12))
                        signalTargetRow("HIDEOUT", "Board attributions",      current: week.hideoutBoardAttributions, target: 1,  unit: "/wk", color: .inkAmber)
                        Divider().background(Color.muted.opacity(0.12))
                        signalBoolRow("FORM",  "Outside-network engagement",  value: week.formOutsideEngagement, color: .inkGreen)
                        Divider().background(Color.muted.opacity(0.12))
                        signalBoolRow("FORGE", "v1 gate passed",              value: week.forgeV1GatePassed,    color: .violetLight)
                    }
                    .background(Color.surface2.opacity(0.5))
                    .clipShape(RoundedRectangle(cornerRadius: 10))
                    ...
                    if !week.oneAdjustment.isEmpty {
                        Rectangle().fill(Color.muted.opacity(0.2)).frame(height: 0.5)
                        VStack(alignment: .leading, spacing: 5) {
                            MonoLabel(text: "ONE ADJUSTMENT · \(week.weekLabel.uppercased())", color: .textMuted, size: 9)
                            Text(week.oneAdjustment)
                                .font(.sora(metrics.scaledSize(12))).foregroundColor(.textPrimary).lineSpacing(2)
                        }
                    }
                } else {
                    Text("No Friday log yet. Signal tab → Friday after close.")
