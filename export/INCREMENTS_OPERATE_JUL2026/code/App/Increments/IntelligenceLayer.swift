...
enum OperatorDoctrine {
    ...
    enum FrictionType: String, CaseIterable {
        ...
        case operationalDisplacement = "OPERATIONAL DISPLACEMENT" // logistics consuming leverage time
    ...
    static let operationalDisplacementDaysWarning = 3       // admin-only mornings before flagging
...
struct ObservedIntelligence {
    ...
    // Admin displacement
    let operationalDisplacementFrequency: Int     // days of 14 where admin consumed but no creative
    let operationalDisplacementRisk: Bool         // >= threshold
    ...
    // Leverage vs operational
    let leverageRatio: Double             // % of completed actions that are creative/analytical
    ...
    var wendyPayloadLines: [String] {
        ...
        if let ft = dominantFrictionType {
            switch ft {
            case .operationalDisplacement:
                lines.append("OBSERVED PATTERN: Admin displacement likely contributing — \(operationalDisplacementFrequency) of last 14 days showed admin without generative follow-through.")
        ...
        if operationalDisplacementRisk && dominantFrictionType != .operationalDisplacement {
            lines.append("OBSERVED PATTERN: Admin displacement — \(operationalDisplacementFrequency) of last 14 days.")
        }
        ...
        if leverageRatio < 0.25 {
            lines.append("OBSERVED PATTERN: \(Int(leverageRatio * 100))% of recent completions are generative. Operational work dominant.")
        }
...
struct ObservedIntelligenceEngine {
    ...
    static func compute(actions: [Action], logs: [DailyLog]) -> ObservedIntelligence {
        ...
        let operationalDisplacementDays = last14.filter { log in
        ...
        if operationalDisplacementDays >= OperatorDoctrine.operationalDisplacementDaysWarning {
            dominantFriction = .operationalDisplacement
        ...
        let leverageRatio = recentCompletions.isEmpty ? 0.5 : Double(generativeCount) / Double(recentCompletions.count)

        // ── Friction signature ───────────────────────────────────────────
        let frictionSig: String
        switch dominantFriction {
        case .operationalDisplacement:
            frictionSig = "Signal: operational displacement likely contributing to morning pattern"
        case .structuralFragmentation:
        ...
        return ObservedIntelligence(
            ...
            operationalDisplacementFrequency: operationalDisplacementDays,
            operationalDisplacementRisk: operationalDisplacementDays >= OperatorDoctrine.operationalDisplacementDaysWarning,
...
nonisolated enum WendyPrompts {
    static let system: String = _WendyPromptStrings.wendy
    static let consult: String = _WendyPromptStrings.consult
}

// Strings live in a nonisolated enum to avoid Swift 6 main-actor isolation on file-scope lets.
private nonisolated enum _WendyPromptStrings {
    static let wendy: String = _wendySystemPromptValue
...
- Primary friction sources: sequencing ambiguity, structural fragmentation, operational displacement, environmental disorder, cleanup debt
...
1. PERSONAL: 5:00 wake target. Morning stack (core → activation → cardio) ~5:10–5:45AM. Mon/Fri: Rod coaching at Panorama 6:15 — compress AM cardio. Commute: 26th floor condo → elevated terrace patio (same building, second floor). Hideout Wed–Fri 7AM–5PM, Sat–Sun 7AM–3PM. Mon–Tue base days. Evening: Forge Breechay hypertrophy ~5:30PM. Sleep target 9:15–9:30PM.
2. BUSINESS: Hideout Miami — neighborhood infrastructure, outdoor elevated terrace, second floor of condo building. Food and coffee are infrastructure; the product is conditions. Solo operator growth phase (started May 13). Track 30-day avg, bands, recurring revenue — not loan urgency. Bands: survival <$520 · stability $590 · comfort $650 · growth $750+.
3. PHYSICAL: Currently ~12–13% body fat, goal 8–10%. Forge Breechay Hypertrophy. Tibia ~95% — late Phase II progressive load. Cardio bike/elliptical; running return path active. Strength ~5:30PM. Rod Panorama Mon/Fri 6:15AM coaching.
...
- OPERATIONAL DISPLACEMENT: logistics consuming morning leverage time
...
class VoicePresence: NSObject, AVSpeechSynthesizerDelegate, @unchecked Sendable {
    ...
    func speakIfWarranted(
        ...
        // ARCH: text intelligence (wendyEnabled) runs independently from voice (voicePresenceEnabled).
        // Voice is off by default — intelligence layer still surfaces observations as text.
        ...
        if let layerAText = PresenceSpeech.observe(context) {
            ...
            // Text observation — always surface when wendyEnabled
            await MainActor.run {
                WendyState.shared.pendingObservation = layerAText
            }
            // Voice — only if voicePresenceEnabled
        ...
        if let layerBText = await generateWendyObservation(
            ...
            let voiceShouldFire = profile.wendyVoiceEnabled && voiceEnabled && canSpeak()
    ...
    private func buildWendyPayload(context: PresenceContext, longitudinal: LongitudinalContext, actions: [Action] = []) -> String {
        ...
        HIDEOUT — SOLO OPERATOR DAY \(longitudinal.hideoutExperimentDay) · WK \(max(1, (longitudinal.hideoutExperimentDay - 1) / 7 + 1)):
        \(recentShifts)
        BAND: \(longitudinal.hideoutPlanningBand) · 30-DAY AVG: $\(String(format: "%.0f", longitudinal.hideoutThirtyDayAvg))/day · STAFF USED: \(longitudinal.hideoutStaffUsedRecently ? "yes" : "no") · LOST SALES: \(longitudinal.hideoutLostSalesRecently ? "yes" : "no")
        GROWTH CONTEXT: Track gap to next band and recurring revenue — not loan framing.\(longitudinal.writingActionRecentlyDone.map { done in "\n        DIRECTION — Writing action (signal move): \(done ? "completed in last 30 days" : "not completed in last 30 days")" } ?? "")
        """
    }
...
class NotificationService {
    ...
    // 5:00 wake architecture: anchor stack ~5:00–5:45AM, hideout 7AM open
    ...
    func scheduleAll(profile: OperatorProfile) {
        ...
        let nudges: [(String, String, Int, Int, Bool, String)] = [
            // 5:00 — morning anchor open (every day)
            ("No phone. First hour starts now.",
             "",
             5, 0, profile.notifCategoryHealth, "increments-0"),
            ...
            // 20:30 — evening anchor (early for 5:00 wake / 9:30PM sleep)
        ...
        // Hydration — 5:00 wake schedule: covers full active window 7AM–7PM
