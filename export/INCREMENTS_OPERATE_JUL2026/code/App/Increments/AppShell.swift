...
struct CustomTabBar: View {
    ...
    // Legacy aliases — map to Park routes via AppState.parkRoute
    static let tabNow = 0
    static let tabToday = 1
    static let tabPhysique = 2
    static let tabHideout = 3
    static let tabSignal = 4
    static let tabYou = 5
    static let tabRecovery = 6
...
func remapTabIndexAfterTrim(_ tab: Int) -> Int {
    remapTabIndexForOperateMode(tab)
}

/// Jul 2026 — 7-tab → Operate + Park. Preserves deep-link intent via parkRoute.
func remapTabIndexForOperateMode(_ tab: Int) -> Int {
    if tab <= CustomTabBar.tabToday { return CustomTabBar.tabOperate }
    return CustomTabBar.tabPark
}

func parkRouteFromLegacyTab(_ tab: Int) -> ParkDestination? {
    switch tab {
    case CustomTabBar.tabNow: return .now
    case CustomTabBar.tabToday: return .today
    case CustomTabBar.tabPhysique: return .physique
    case CustomTabBar.tabHideout: return .hideout
    case CustomTabBar.tabSignal: return .signal
    case CustomTabBar.tabYou: return .you
    case CustomTabBar.tabRecovery: return .recovery
    default: return nil
    }
}
...
struct RootView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            let legacyTab = state.selectedTab
            state.selectedTab = remapTabIndexForOperateMode(legacyTab)
            if legacyTab > CustomTabBar.tabToday, let route = parkRouteFromLegacyTab(legacyTab) {
                state.parkRoute = route
            }
            if profiles.isEmpty {
