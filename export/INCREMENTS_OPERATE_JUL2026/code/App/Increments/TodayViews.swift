...
struct HomeView: View {
    ...
    // Growth system completion state — mirrors HideoutViews AppStorage
    @AppStorage("growth_card_designed")   private var growthCardDesigned   = false

    // Housing optimization state — mirrors CapitalViews AppStorage
    ...
    @AppStorage("growth_sign_ordered")    private var growthSignOrdered    = false
    @AppStorage("growth_sign_placed")     private var growthSignPlaced     = false
    ...
    var hideoutStatusLine: String {
        ...
        let closeHour = (weekday == 1 || weekday == 7) ? 15 : 17
        if hour < 7      { return "Opens at 7. Prep window." }
        if hour < closeHour { return "Shift active. Log after close." }
        return "Shift closed. Log now."  // post-close: action-oriented, not passive
    }

    // Next incomplete growth system action for Today surface
    var nextGrowthAction: String? {
        if !growthCardDesigned  { return "Growth · Design Watermarc card in Canva" }
        if !growthGBPReviews    { return "Growth · Respond to 4 unread GBP reviews" }
        if !growthGBPProfile    { return "Growth · Complete GBP profile (hours, photos, menu)" }
        if !growthCardPrinted   { return "Growth · Place print order — Vistaprint, 50 cards" }
        if !growthSignOrdered   { return "Growth · Order A-frame sign ($80–180)" }
        if !growthGBPPhotos     { return "Growth · Upload 5 photos to GBP (patio first)" }
        if !growthCardDelivered { return "Growth · Deliver cards to Watermarc front desk" }
        if !growthSignPlaced    { return "Growth · Place A-frame at street-level entrance" }
        if !growthVideoFilmed   { return "Growth · Film Monday clip before opening (7-shot list)" }
        if !growthVideoPosted   { return "Growth · Post to GBP + Reels + TikTok" }
        return nil
    }
    ...
    // Next incomplete housing action for Today surface
    var nextHousingAction: String? {
        guard !housingMoveInSet else { return nil }          // done — disappears
        if !housingDecisionMade  { return "Housing · Pick approach in Capital tab" }
        if !housingPostDrafted   { return "Housing · Draft listing — 3 sentences, one photo" }
        if !housingOutreachDone  { return "Housing · Post to Furnished Finder + Edgewater groups" }
        if !housingScreeningDone { return "Housing · Schedule 2–3 candidate screenings" }
        return "Housing · Set move-in date"
    }
            ...
            if metrics.isIPad {
                        ...
                        ScrollView(showsIndicators: false) {
                            VStack(spacing: metrics.blockSpacing) {
                                ...
                                if todayIsHideoutDay {
                                    ...
                                    .onTapGesture {
                                        state.parkRoute = .hideout
                                        state.selectedTab = CustomTabBar.tabPark
                                    }
                                            ...
                                            VStack(alignment: .leading, spacing: metrics.rowSpacing) {
                                                ...
                                                if sleepHour < 10 {
                                                    if let sq = sleepYesterdayLog?.sleepQuality, !sq.isEmpty {
                                                        ...
                                                        Text("5:00 AM wake · 6H 15MIN window")
                                                            .font(metrics.fontSora(13, weight: .light))
                                                            .foregroundColor(.textSecond)
                                                    }
                ...
                ScrollView(showsIndicators: false) {
                    ...
                    VStack(spacing: metrics.blockSpacing) {
                        ...
                        let sleepShutdownMin = 20 * 60 + 30
                        let sleepYesterdayLog = logs.first(where: { Calendar.current.isDateInYesterday($0.date) })
                        let showSleepMorning = sleepHour < 10
                        let showSleepEvening = sleepTotalMin >= sleepShutdownMin
...
struct TodayView: View {
    ...
    @State private var selectedTodaySeg = 0   // 0=Rail 1=Protocols 2=Log
    @State private var showProtocolsSheet = false
    ...
    var currentTimeGroupID: String {
        ...
        switch h {
        case 0..<12:  return TimeGroup.morning.rawValue
        case 12..<17: return TimeGroup.afternoon.rawValue
        default:      return TimeGroup.evening.rawValue
        }
    }
    ...
    var readinessChipText: String {
        ...
        return "Pattern window opens in \(remaining) day\(remaining == 1 ? "" : "s"). Keep logging."
    }
    ...
    var todayDoctrine: String {
        ...
        // Only surface when there's actual operational signal worth reading.
        // No encouragement. No affirmations. Either useful or nothing.

        // Pre-open: participation gap is the one worth flagging
        if hour < 10 && participationQuietDays >= 5 {
            return "Participation quiet for \(participationQuietDays) days."
        }

        // Midday nothing done — actionable, not motivational
        if hour >= 12 && hour < 17 && completedCount == 0 {
            return "Nothing closed yet. First action is pending."
        }

        // Sunday close — the one contextual moment that's operationally real
        if isSunday && hour >= 18 {
            return "Weekly Reset tonight. Close the week before it becomes next week's drag."
        }

        // Default: silence. The rail speaks for itself.
        return ""
    }
    ...
    func todayIPadReflectionColumn() -> some View {
        VStack(alignment: .leading, spacing: metrics.sectionGap) {
            ...
            Button(action: { showProtocolsSheet = true }) {
                CardView(style: .secondary) {
                    HStack {
                        VStack(alignment: .leading, spacing: metrics.rowSpacing) {
                            MonoLabel(text: "PROTOCOLS", color: .violetLight, size: 9)
                            Text("Sessions · maintenance · doctrine rails")
                                .font(metrics.fontSora(13, weight: .light))
                                .foregroundColor(.textSecond)
                        }
                        Spacer()
    ...
    var body: some View {
                        ...
                        if !todayActions.isEmpty {
                            VStack(alignment: .leading, spacing: metrics.blockSpacing) {
                                ...
                                ForEach(pendingByTimeGroup, id: \.group.rawValue) { bucket in
                                            ...
                                            VStack(spacing: 0) {
                                                ForEach(Array(bucket.actions.enumerated()), id: \.element.id) { i, action in
                                                    ...
                                                    if i < bucket.actions.count - 1 {
                                                        ...
                                                        HStack(spacing: 0) {
                                                            Rectangle()
                                                                .fill(action.system.color.opacity(0.15))
                                    ...
                                    .id(bucket.group.rawValue) // scroll anchor
                                }
        ...
        .sheet(isPresented: $showProtocolsSheet) {
            NavigationStack {
                ZStack {
                    AtmosphericBackground()
                    ScrollView(showsIndicators: false) {
                ...
                .toolbar {
                    ToolbarItem(placement: .topBarTrailing) {
                        Button("Done") { showProtocolsSheet = false }
        ...
        .onAppear {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                appeared = true
                // Time-aware scroll: jump to current time band if past morning
                let h = Calendar.current.component(.hour, from: Date())
                if h >= 12 {
                    withAnimation(.easeOut(duration: 0.4)) {
                        scrollProxy?.scrollTo(currentTimeGroupID, anchor: .top)
                    }
                }
            }
            ...
            if let p = profiles.first {
                // Always build intelligence context — voice is separate from intelligence
...
// MARK: - PROTOCOLS VIEW EMBED

struct ProtocolsViewEmbed: View {
    @Environment(\.appMetrics) private var metrics
    @Query private var allSessions: [Session]
    @State private var selectedSession: Session? = nil

    var activeSessions: [Session] { allSessions.filter(\.isActive) }

    // Time-ordered sequence — same logic as the Rail
    var orderedProtocols: [Session] {
        let hour = Calendar.current.component(.hour, from: Date())
        let isSunday = Calendar.current.component(.weekday, from: Date()) == 1

        // Score each session by how relevant it is right now
        func score(_ s: Session) -> Int {
            let t = s.title.lowercased()
            let isMorning = t.contains("morning") || t.contains("whole human")
            let isTraining = t.contains("warmup") || t.contains("pre-lift") || t.contains("laundry")
            let isEvening = t.contains("shutdown") || t.contains("preservation") || t.contains("evening")
            let isWeekly = s.recurrence == .weekly

            // Sunday-only sessions hidden on other days
            if isWeekly && (t.contains("weekly reset") || t.contains("make something")) && !isSunday {
                return 999
            }
            ...
            if hour < 11 {
                ...
                if isWeekly   { return 3 }
            } else if hour < 16 {
                if isTraining { return 0 }
                if isMorning  { return 1 }
                if isEvening  { return 2 }
                if isWeekly   { return 3 }
            } else if hour < 21 {
                if isEvening  { return 0 }
                if isTraining { return 1 }
                if isWeekly && isSunday { return 2 }
                if isMorning  { return 3 }
            } else {
                if isEvening  { return 0 }
                if isWeekly && isSunday { return 1 }
        ...
        return activeSessions
            ...
            .sorted {
                ...
                // Within same group: incomplete first, then by title
...
// MARK: - PROTOCOL DOOR
// Minimal entry point. Name + timing cue + step count. Tap to enter.
...
struct ProtocolDoor: View {
    ...
    var timingLabel: String {
        ...
        if t.contains("morning") || t.contains("whole human") { return "4–5 AM" }
        if t.contains("warmup") || t.contains("pre-lift") { return "5:15 PM" }
        if t.contains("shutdown") { return "8:25 PM" }
        if t.contains("preservation") { return "After screens off" }
        if t.contains("weekly reset") { return "Sunday 7 PM" }
        if t.contains("laundry") { return "As needed" }
        if !session.cue.isEmpty {
            // Extract just the time if it starts with one
            let parts = session.cue.components(separatedBy: "—")
            return parts.first?.trimmingCharacters(in: .whitespaces) ?? session.cue
        }
        return session.cue
    }
        ...
        Button(action: onTap) {
            HStack(spacing: metrics.blockSpacing) {
                ...
                RoundedRectangle(cornerRadius: 2)
                    .fill(
                        LinearGradient(
                            colors: [session.system.color, session.system.color.opacity(0.3)],
                            startPoint: .top, endPoint: .bottom
                        )
                    )
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 12)
                    .strokeBorder(
                        session.isCompleted ? Color.inkGreen.opacity(0.12) : Color.white.opacity(0.04),
                        lineWidth: 0.5
                    )
            )
        }
        .buttonStyle(ProtocolDoorButtonStyle())
    }
}

struct ProtocolDoorButtonStyle: ButtonStyle {
...
struct CollapsedCapacityChip: View {
    ...
    var body: some View {
        if let es = state.todayEnergyState {
            ...
            .background(Color.surface2.opacity(0.7))
            .clipShape(RoundedRectangle(cornerRadius: 10))

            if showExpanded {
                EnergyStateInputCard(state: state)
                    .transition(.opacity.combined(with: .move(edge: .top)))
            }
        }
    }
    ...
    var body: some View {
        CardView(style: .secondary) {
            if weather.isLoading && weather.snapshot == nil {
                ...
                HStack {
                    MonoLabel(text: "MIAMI · OFFLINE", color: .textMuted, size: 10)
                    Spacer()
                }
                .frame(minHeight: 40)
            }
