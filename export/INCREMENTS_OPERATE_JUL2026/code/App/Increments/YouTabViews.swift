...
struct VenturesTabView: View {
    ...
    var body: some View {
        VStack(spacing: metrics.blockSpacing) {
            ventureCard(
                ...
                kickerColor: .warm,
                status: "Active · Solo operator · Growth phase",
                headline: "Neighborhood infrastructure. Edgewater. Six years. The house Brice stewards.",
                body: "Hideout is a 4.7-star outdoor terrace on the Edgewater corridor. The product is conditions — food and coffee are infrastructure, not the point. Regulars, open air, six years of reputation. The current constraint is threshold conversion and residential memory encoding: people who find it love it. The bottleneck is find.\n\nSolo operator mode (since May 13): track 30-day avg, planning bands, recurring revenue, and growth systems — not loan urgency. Stability target remains $590/day average.\n\nHideout App (separate build): customer relationship capture — repeat orders, Sunday lineup, private events, B2B gallons. INCREMENTS Hideout tab tracks solo operator growth; the app holds demand the floor cannot keep in memory.",
    ...
    @ViewBuilder
    private func ventureGateRows(id: String) -> some View {
        switch id {
        case "form":
            gateStatusRow(label: "FORM distribution", cleared: true, detail: "Active · coaching surfaces")
        case "forge":
            gateStatusRow(label: "Forge v1 gate", cleared: forgeGateCleared,
                          detail: forgeGateCleared ? "Cleared · distribution allowed" : "Pending · product-readiness only")
        default:
            EmptyView()
        }
    }

    private func gateStatusRow(label: String, cleared: Bool, detail: String) -> some View {
        HStack(spacing: metrics.scaledSize(10)) {
            Circle()
                .fill(cleared ? Color.inkGreen : Color.inkAmber)
                .frame(width: metrics.scaledSize(6), height: metrics.scaledSize(6))
            VStack(alignment: .leading, spacing: 2) {
                MonoLabel(text: label.uppercased(), color: .textMuted, size: 9)
                Text(detail)
                    .font(.mono(metrics.monoSmall))
        ...
        .background(Color.surface2.opacity(0.45))
    ...
    private var hideoutDistribution: some View {
        VStack(alignment: .leading, spacing: metrics.blockSpacing) {
            ...
            ventureProseBlock(
                "Solo operator mode makes nervous-system cost visible: without staff, every transaction is a direct exchange. The question is not only whether you hit the revenue target but what hitting it cost, and whether the cost was worth it.",
                color: .warm
            )
            ...
            ventureProseBlock(
                "Hideout App · customer surface (Documents/HideoutApp): reorder, Sunday, event requests, B2B gallons — relationship compression, not ordering theater. Pre-Square on device. Admin: lineup, events queue, catering. This tab = solo operator growth ops; the app = captured demand.",
                color: .warm
            )
        }
    }
    ...
    var body: some View {
        CardView(style: .secondary) {
            VStack(alignment: .leading, spacing: metrics.blockSpacing) {
                ...
                intelProse(
                    "Personal running returns gradually. FORM content comes through coaching, pacing, and the intelligence layer: Tuesday threshold, Saturday long run, Sony, one ledger sentence."
                )
