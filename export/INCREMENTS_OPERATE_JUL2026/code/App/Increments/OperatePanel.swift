...
enum OperateParsing {
    ...
    static func spineLineForToday(_ spine: String) -> String? {
        ...
        return lines(from: spine).first { line in
            let lower = line.lowercased()
            return lower.hasPrefix(key)
                || lower.contains(" \(key)")
                || lower.contains("· \(key)")
                || lower.contains("— \(key)")
        }
...
struct OperateTabView: View {
    ...
    private var editFieldsBlock: some View {
        VStack(alignment: .leading, spacing: metrics.operateSectionGap) {
            operateSection(title: "MONEY", accent: .warm) {
                ...
                operateField(label: "DUE", hint: "Item · date · amount — next 14 days.", text: $due, minHeight: 96)
                operateField(label: "IN", hint: "Who · amount · next collection action.", text: $incoming, minHeight: 96)
            }
    ...
    private var reviewButton: some View {
        Button {
            lastWeeklyReview = OperateParsing.reviewStamp()
            #if os(iOS)
