...
struct AppMetrics {
    ...
    var headlineSize:  CGFloat { (isIPad ? 20 : 17) * typeScale }
    var bodySize:      CGFloat { (isIPad ? 17 : 14) * typeScale }
    ...
    var captionSize:   CGFloat { (isIPad ? 15 : 12) * typeScale }
    var monoSize:      CGFloat { (isIPad ? 14 : 11) * typeScale }
    var monoSmall:     CGFloat { (isIPad ? 12 : 10) * typeScale }

    // Operate tab — arm's-length morning read (Jul 2026)
    var operateTitleSize:    CGFloat { (isIPad ? 32 : 22) * typeScale }
    var operateTaskSize:     CGFloat { (isIPad ? 22 : 17) * typeScale }
    var operateBodySize:     CGFloat { (isIPad ? 19 : 16) * typeScale }
    var operateMoneySize:    CGFloat { (isIPad ? 28 : 22) * typeScale }
    var operateCaptionSize:  CGFloat { (isIPad ? 16 : 13) * typeScale }
    var operateMonoSize:     CGFloat { (isIPad ? 13 : 11) * typeScale }
    var operateCardPad:      CGFloat { (isIPad ? 24 : 18) * typeScale }
    var operateSectionGap:   CGFloat { (isIPad ? 28 : 20) * vScale }
